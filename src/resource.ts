...
const methods = {
	async custom(resource, operation) {
		let relativePath = await resource.root.parent.system.file.join(
			resource.root.parent.settings.folders.file,
			operation.custom.path
		);
		let absolutePath = await resource.root.parent.system.file.join(
			resource.root.parent.system.rootDir,
			relativePath
		);
		let filePath = await resource.root.parent.system.file.join(
			absolutePath,
			operation.custom.name
		);
		operation._out = {
			data: JSON.stringify(
				await require(filePath)(resource, operation)
			) /* eslint-disable-line global-require */, // In-line require suits the needs and logic
			pType: "??",
	...
	async file(resource, operation) {
		...
		operation._out = {
			data: await resource.root.parent.system.file.getFile(
				filePath,
				operation.file.name
			),
			pType: "buffer",
	...
	async md(resource, operation) {
		/* eslint-disable-line require-await */ // Preserving async throught directives
	...
	async njk(resource, operation) {
		let path = await resource.root.parent.system.file.join(
			resource.root.parent.settings.folders.file,
			operation.njk.path
		);
		operation._out = {
			data: await resource.root.parent.app.njk(
				path,
				operation.njk.name,
				operation.hasOwnProperty("_with") ? operation._with : null
			),
			pType: "string",
			...
			switch (operation.out) {
				...
				case "property":
					data = rawData.map(function(result) {
						...
						return {
							data: JSON.stringify(
								result.data.hasOwnProperty(operation._with)
									? result.data[operation._with]
									: ""
							),
							lType: result.lType,
							pType: result.pType
...
class ResourceContext extends system.AtomicLock {
	constructor(appOrParent, name, inData) {
		...
		if (appOrParent instanceof ResourceContext) {
			...
			// NOTE: Instance of App; since this file is to be called from App only, we assume there is no other instanceof possibility
			this.depth = 0;
			this.root = this;
	...
	async process() {
		...
		this.data.forEach(operation => {
			...
			for (let directive in operation) {
				// For each directive in the data array element
				if (directives.primary.includes(directive)) {
					// Primary directives
					this.directives.primary.push(() => {
						// Arrow, since this used for directives
						this.lock();
					...
					this.directives.secondary.push(() =>
						methods[directive](this, operation)
					);
		...
		if (this.directives.out.length == 0) {
			...
			this.directives.out.push(() =>
				methods.out(this, this.data[this.data.length - 1])
			);
		}
