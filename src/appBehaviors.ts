	...
	// Initialize resources
	{
		system_load: that => {
			...
			(async () => {
				...
				let rcFiles = await that.system.file.list(
					rcFolder,
					that.system.file.filter.isFile
				);
				...
				let dbFiles = await that.system.file.list(
					dbFolder,
					that.system.file.filter.isFile
				);
				...
				dbFiles.forEach(function(dbFile) {
					dbResults.push(
						new Promise(function(resolve, reject) {
							that.system.file
								.toAbsolute(dbFolder, dbFile)
								.then(function(dbPath) {
									// Open database connection
									that.app.db[path.parse(dbFile).name] = new sqlite3.Database(
										dbPath,
										err => {
											if (err) {
												reject(err);
